...
public class Query
{
  ...
  private static final String CLEAR_TABLES = "DELETE FROM reservations; DELETE FROM users;";
  ...
  private static final String FIND_MATCHING_RESERVATIONS_FID1 = "SELECT COUNT(*) FROM reservations WHERE fid1 = ?";
  private PreparedStatement findMatchfid1;
  
  private static final String FIND_MATCHING_RESERVATIONS_FID2 = "SELECT COUNT(*) FROM reservations WHERE fid2 = ?";
  private PreparedStatement findMatchfid2;
  ...
  public void prepareStatements() throws Exception
  {
    ...
    findMatchfid1 = conn.prepareStatement(FIND_MATCHING_RESERVATIONS_FID1);
    findMatchfid2 = conn.prepareStatement(FIND_MATCHING_RESERVATIONS_FID2);
  }
		  ...
		  for (Itinerary i: itineraries) {
			  if (i.id == itineraryId) {
				  // check if user has other bookings on the same day
					...
					// check if there is room on the plane
					
					// four matches: our first flight with booked flight1, first with booked flight2,
					// 				our second flight with booked flight1, second with booked flight2
					findMatchfid1.clearParameters();
					findMatchfid1.setInt(1, i.flight1.fid);
					ResultSet numMatches1 = findMatchfid1.executeQuery();
					numMatches1.next();
					int matching1 = numMatches1.getInt(1);
					numMatches1.close();
					
					findMatchfid2.clearParameters();
					findMatchfid2.setInt(1, i.flight1.fid);
					ResultSet numMatches2 = findMatchfid2.executeQuery();
					numMatches2.next();
					int matching2 = numMatches2.getInt(1);
					numMatches2.close();
					
					int flight1Matches = matching1 + matching2;
					if (i.flight1.capacity <= flight1Matches) {
						rollbackTransaction();
					...
					if (i.flight2 != null) {
						findMatchfid1.clearParameters();
						findMatchfid1.setInt(1, i.flight2.fid);
						ResultSet numMatches2_1 = findMatchfid1.executeQuery();
						numMatches2_1.next();
						int matching2_1 = numMatches2_1.getInt(1);
						numMatches2_1.close();
						
						findMatchfid2.clearParameters();
						findMatchfid2.setInt(1, i.flight2.fid);
						ResultSet numMatches2_2 = findMatchfid2.executeQuery();
						numMatches2_2.next();
						int matching2_2 = numMatches2_2.getInt(1);
						numMatches2_2.close();
						
						int flight2Matches = matching2_1 + matching2_2;
						System.err.println("flight2Matches: " + flight2Matches);
						System.err.println("flight2 capacity: " + i.flight2.capacity);
						
						if (i.flight2.capacity <= flight2Matches) {
							rollbackTransaction();
							return "Booking failed\n";
						}
					}
